...
st.set_page_config(
    page_title="VNM Financial Risk Dashboard",
    layout="wide",
...
# 2. HÀM TẢI TÀI SẢN HỌC MÁY (Bọc try-except chuẩn chỉ ở đây)
@st.cache_resource
def load_ml_assets():
    try:
        # Đi thẳng từ thư mục gốc của Repo vào thư mục BTL.AI lấy file
        model = joblib.load("BTL.AI/logistic_model.pkl")
        scaler = joblib.load("BTL.AI/scaler.pkl")
        return model, scaler
    except FileNotFoundError:
        st.error(
            "❌ Hệ thống không tìm thấy file bộ não học máy `logistic_model.pkl` hoặc `scaler.pkl`. Hãy chắc chắn rằng bạn đã chạy file `Train_model.py` trước đó và đẩy file lên đúng thư mục `BTL.AI` trên GitHub."
        )
        return None, None


# Thực thi tải mô hình
model, scaler = load_ml_assets()

# --- TIÊU ĐỀ CHÍNH ---
st.markdown(
    "<h1 style='text-align: center; color: #1E3A8A; font-family: sans-serif;'>🏛️ HỆ THỐNG QUẢN TRỊ RỦI RO TÀI CHÍNH CHIẾN LƯỢC - VINAMILK (VNM)</h1>",
    unsafe_allow_html=True,
)
st.markdown(
    "<p style='text-align: center; color: #64748B; font-size: 16px;'>Mô hình toán học kết hợp Học máy phân tích đa chiều Chuỗi chỉ số theo thời gian</p>",
    unsafe_allow_html=True,
)
st.write("---")
...
with st.container():
    ...
    with col1:
        ...
        net_profit = st.number_input(
            "2. Lợi nhuận sau thuế (VND)",
            value=2200000000000.0,
            step=10000000000.0,
            format="%.0f",
        )
        cash_flow = st.number_input(
            "3. Dòng tiền thuần từ HĐKD (VND)",
            value=2500000000000.0,
            step=10000000000.0,
            format="%.0f",
        )
    ...
    with col2:
        ...
        total_assets = st.number_input(
            "4. Tổng tài sản (VND)",
            min_value=1.0,
            value=52000000000000.0,
            step=100000000000.0,
            format="%.0f",
        )
        total_debt = st.number_input(
            "5. Tổng nợ phải trả (VND)",
            min_value=0.0,
            value=15000000000000.0,
            step=100000000000.0,
            format="%.0f",
        ...
        equity = st.number_input(
            "6. Vốn chủ sở hữu (VND)",
            min_value=1.0,
            value=37000000000000.0,
            step=100000000000.0,
            format="%.0f",
        )
    ...
    with col3:
        ...
        current_ratio = st.number_input(
            "7. Hệ số thanh toán hiện hành (Lần)", min_value=0.0, value=2.2, step=0.1
        )
        roa = st.number_input(
            "8. Tỷ suất sinh lời trên Tài sản - ROA (%)", value=4.2, step=0.1
        )
        roe = st.number_input(
            "9. Tỷ suất sinh lời trên Vốn CSH - ROE (%)", value=5.9, step=0.1
        )

st.write("---")

# Chỉ chạy phân tích nếu load được model và scaler thành công
if model is not None and scaler is not None:
    # --- THUẬT TOÁN XỬ LÝ DỮ LIỆU ĐỘNG ---
    calculated_debt_ratio = total_debt / total_assets

    # Dự báo Core Probability từ mô hình học máy gốc
    ml_input = np.array(
        [[revenue, net_profit, calculated_debt_ratio, roa / 100, roe / 100]]
    )
    ml_input_scaled = scaler.transform(ml_input)
    base_probability = model.predict_proba(ml_input_scaled)[0][1]

    # HỆ CHUYÊN GIA (EXPERT LAYER)
    penalty_score = 0.0
    if cash_flow < 0:
        penalty_score += 0.15
    ...
    if st.button("🔥 CHẠY ENGINE PHÂN TÍCH CHUYÊN SÂU", type="primary"):
        ...
        with m_col1:
            st.metric(
                "Tỷ số nợ thực tế",
                f"{calculated_debt_ratio:.2%}",
                delta="- An toàn" if calculated_debt_ratio < 0.4 else "+ Rủi ro cao",
                delta_color="inverse",
            )
        with m_col2:
            st.metric(
                "Chất lượng dòng tiền",
                f"{cash_flow/15000000000000*100:.1f}%",
                "Dương" if cash_flow > 0 else "Âm nghiêm trọng",
            )
        with m_col3:
            st.metric(
                "Hệ số thanh khoản",
                f"{current_ratio:.2f} lần",
                "Đạt chuẩn" if current_ratio >= 1.5 else "Yếu",
            )
        with m_col4:
        ...
        with g_col1:
            st.markdown(
                "<h4 style='text-align: center; color: #1E3A8A;'>⏱️ Đồng hồ Cảnh báo Áp lực Rủi ro Tài chính Tổng hợp</h4>",
                unsafe_allow_html=True,
            )
            fig_gauge = go.Figure(
                go.Indicator(
        ...
        with g_col2:
            st.markdown(
                "<h4 style='text-align: center; color: #1E3A8A;'>📊 Trọng số Ảnh hưởng của các Nhóm Biến Tài chính</h4>",
                unsafe_allow_html=True,
            )
            try:
            ...
            df_imp = pd.DataFrame(
                {"Biến": features, "Mức độ ảnh hưởng (%)": importance}
            ).sort_values("Mức độ ảnh hưởng (%)")
            fig_bar = px.bar(
                df_imp,
                x="Mức độ ảnh hưởng (%)",
                y="Biến",
                orientation="h",
                color="Mức độ ảnh hưởng (%)",
        ...
        # =========================================================================
        # 🧠 THUẬT TOÁN BIÊN SOẠN BÁO CÁO CHIẾN LƯỢC ĐỘNG TỰ ĐỘNG
        # =========================================================================
        ...
        st.markdown(
            f"### 🧠 Báo cáo Tư vấn Chiến lược từ Chuyên gia AI (Trạng thái rủi ro hệ thống: {risk_label})"
        )
    ...
    st.warning(
        "⚠️ Không thể chạy ứng dụng do thiếu các tệp tin mô hình học máy cốt lõi."
    )
